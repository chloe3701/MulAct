...
def declare_constraints_p_smr_physical_flux(
    ...
    def C_prod_smr_0_rule(m, i, t):
        return m.Q_energie_total[i, t] == sum(
            m.Q_energie[i, e, t] for e in Actors[i].internal_struct.energy_sources
        )
    ...
    def C_prod_smr_1_rule(m, i, t):
        return (
            m.Q_H2_prod[i, t]
            == m.Q_energie_total[i, t] * Actors[i].internal_struct.SMR.efficiency
        )
...
def declare_constraints_p_smr_environmental(
    ...
    def C_prod_smr_9_rule(m, i, t):
        ...
        return (
            m.Emission_vaporeformage[i, t] == m.Q_H2_prod[i, t] * Impact_vaporeformage
        )
    ...
    if optim_CO2_heure:
        ...
        def C_prod_smr_11_rule(m, i, t):
            return (
                m.Impact_prod[i, t]
                <= Actors[i].internal_struct.impact_max * m.Q_H2_prod[i, t]
            )
    ...
    # Si contrainte en moyenne
        ...
        def C_prod_smr_11_rule(m, i):
            return sum(m.Impact_prod[i, t] for t in Time) <= Actors[
                i
            ].internal_struct.impact_max * sum(m.Q_H2_prod[i, t] for t in Time)
